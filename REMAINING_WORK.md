...
| MyListings (Portföyüm) | ❌ | `data/properties` + `AppContext` listeleri (sadece bellek) |
| Favorites (Listeler) | ❌ | `data/properties` + `AppContext` (sadece bellek) |
...
| 4 | **Lists / Favorites** | Bağlı değil | Özel listeler (Favoriler, Portföyüm) + öğe başına **not**; `AppContext`'i backend'e taşı (`ListItem.note` eksik) |
...
3. Customers + CustomerListings + Lists/Favorites
