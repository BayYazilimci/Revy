...
async function loadImageBase64(url) {
  return new Promise(resolve => {
    ...
    img.onload = () => {
      ...
      try { resolve({ data: canvas.toDataURL('image/jpeg', 0.92), w: img.naturalWidth, h: img.naturalHeight }) }
      catch { resolve(null) }
    }
...
export async function generatePDF(prop, sections, nearbyData, evdsData) {
  ...
  pdf.addFileToVFS('DejaVu.ttf', DEJAVU_REGULAR)
  pdf.addFont('DejaVu.ttf', FONT, 'normal')
  pdf.addFileToVFS('DejaVu-Bold.ttf', DEJAVU_BOLD)
