...
export default function ListingDetail() {
  ...
  return (
    <div className="flex flex-col min-h-0">
      ...
      <div ref={presentationRef} style={{ position: 'fixed', left: 0, top: 0, width: '794px', zIndex: -1, opacity: 0.01, pointerEvents: 'none', background: '#fff', fontFamily: 'Plus Jakarta Sans, sans-serif', overflow: 'hidden' }}>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-5">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-cardBorder">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-gray-400">Profesyonel Sunum</p>
                <p className="text-sm font-bold" style={{ color: '#1e1b2e' }}>Bu ilan için PDF sunumu oluştur</p>
              </div>
              <button
                className="btn px-4 py-2.5 rounded-xl text-[11px] font-extrabold shadow-lg flex items-center justify-center gap-1.5 flex-shrink-0"
                style={{ background: '#e3d10d', color: '#1e1b2e', boxShadow: '0 8px 24px rgba(227,209,13,.25)' }}
                onClick={generatePresentation}
                disabled={generating}
              >
                {generating ? <Loader2 size={13} className="animate-spin" /> : <FileText size={13} />}
                {generating ? 'Oluşturuluyor...' : 'Sunum Oluştur'}
              </button>
            </div>
          </div>
