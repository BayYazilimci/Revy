...
import {
  ...
  FileText, Loader2, CheckCircle, Download
} from 'lucide-react'
...
export default function MyListings() {
  ...
  const [showPdfSuccess, setShowPdfSuccess] = useState(null)
  ...
  const generatePresentation = async (prop) => {
    ...
    try {
      ...
      const element = document.getElementById('pdf-template-' + prop.id)
      if (!element) return
  ...
  const downloadPdf = () => {
    if (pdfReady) {
      const a = document.createElement('a')
      a.href = pdfReady
      a.download = 'Sunum.pdf'
      a.click()
    }
  }

  const closePdfSuccess = () => {
    setShowPdfSuccess(null)
    if (pdfReady) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
              {listingProps.map((prop, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        <button
                          className="py-2.5 px-4 rounded-xl text-[11px] font-extrabold btn flex items-center justify-center gap-1.5 transition-all"
                          style={{ color: '#1e1b2e', background: '#f0ece6' }}
                          onClick={() => generatePresentation(prop)}
                          disabled={generatingPdf === prop.id}
                        >
                          {generatingPdf === prop.id ? <Loader2 size={13} className="animate-spin" /> : <FileText size={13} />}
                        </button>
                        <button
                  ...
                  {/* PDF SUCCESS POPUP */}
                  {showPdfSuccess === prop.id && pdfReady && (
